...
export class PreviewDialogComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: {image: NgxImage, images:NgxImage[], currentImageIndex: number, imagesCount: number}) { }
  ...
  nextImage(currentImageIndex:number): void {
    const index = currentImageIndex + 1 === this.data.imagesCount ? 0 : currentImageIndex + 1;
    this.currentImage = this.data.images[index];
    this.currentImageIndex = index;
  }
  
  prevImage(currentImageIndex:number): void {
    const index = currentImageIndex <= 0 ? this.data.imagesCount - 1 : currentImageIndex - 1;
    this.currentImage = this.data.images[index];
    this.currentImageIndex = index
  }
